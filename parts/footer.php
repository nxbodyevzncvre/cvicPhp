<?php
include_once 'functions.php';
$footerMenu = getMenuData("footer");
$theme = isset($_GET['theme']) && $_GET['theme'] === 'dark' ? 'dark' : 'light';
?>

<footer class="container <?php echo $theme === 'dark' ? 'bg-dark text-white' : ''; ?>">
    <div class="row">
        <div class="col-25">
            <h4>Kto sme</h4>
            <p>Laboris duis ut est fugiat et reprehenderit magna labore aute.</p>
        </div>
        <div class="col-25 text-left">
            <h4>Kontaktujte nás</h4>
            <p><i class="fa fa-envelope"></i> <a href="mailto:livia.kelebercova@gmail.com" class="<?php echo $theme === 'dark' ? 'dark' : 'light'; ?>">livia.kelebercova@gmail.com</a></p>
            <p><i class="fa fa-phone"></i> <a href="tel:0909500600" class="<?php echo $theme === 'dark' ? 'dark' : 'light'; ?>">0909500600</a></p>
        </div>
        <div class="col-25">
            <h4>Rýchle odkazy</h4>
            <ul>
                <?php foreach ($footerMenu as $menuItem): ?>
                    <li><a href="<?php echo $menuItem['path']; ?>?theme=<?php echo $theme; ?>"><?php echo $menuItem['name']; ?></a></li>
                <?php endforeach; ?>
            </ul>
        </div>
        <div class="col-25">
            <h4>Nájdete nás</h4>
            <iframe src="https://www.google.com/maps/embed?pb=!1m14!1m8!1m3!1d10614.839764656655!2d18.0910518!3d48.3084298!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x0%3A0xba2bad032d96b960!2sFakulta%20pr%C3%ADrodn%C3%BDch%20vied%20a%20informatiky!5e0!3m2!1ssk!2ssk!4v1669307792855!5m2!1ssk!2ssk" width="300" height="150" style="border:0;" allowfullscreen="" loading="lazy"></iframe>
        </div>
    </div>
    <div class="row">
        Created and designed by Lívia
    </div>
</footer>